import React from 'react';
import { Link } from 'react-router-dom';
import { Lock } from 'lucide-react';

const GithubIcon = ({ size = 16, className = '' }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
    <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" />
  </svg>
);

const PlayIcon = ({ className = 'w-4 h-4' }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M3.18 23.76a2 2 0 0 0 2.05-.19l13-7.5-2.83-2.83-12.22 10.52zm16.12-9.93 3.15-1.82a1.5 1.5 0 0 0 0-2.6L19.3 7.59l-3.12 3.12 3.12 3.12zM3.18.24A2 2 0 0 0 2 2v20a2 2 0 0 0 1.18 1.76L15.4 12 3.18.24zm13 11.88-2.83-2.83L3.18.24l13.12 11.88z" />
  </svg>
);

export default function Footer() {
  const year = new Date().getFullYear();

  const handleNav = (href) => {
    document.querySelector(href)?.scrollIntoView({ behavior: 'smooth' });
  };

  return (
    <footer
      className="relative overflow-hidden border-t border-gold/10"
      style={{ background: '#080f18' }}
    >
      {/* Top gradient line */}
      <div
        className="absolute top-0 left-0 right-0 h-px"
        style={{ background: 'linear-gradient(90deg, transparent, #c9a84c, transparent)' }}
      />

      <div className="max-w-7xl mx-auto px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-12">
          {/* Brand */}
          <div>
            <div className="flex items-center gap-3 mb-4">
              <img src="/logo.png" alt="Noor Logo" className="w-10 h-10 object-contain" />
              <div>
                <span className="font-amiri text-xl gold-text font-bold">نور</span>
                <span className="ml-2 text-white font-semibold">Noor</span>
              </div>
            </div>
            <p className="text-slate-500 text-sm leading-relaxed max-w-xs">
              Your daily Islamic companion. Free, open-source, and built with love for the Muslim community.
            </p>
          </div>

          {/* Quick links */}
          <div>
            <h4 className="text-white font-semibold mb-4 text-sm uppercase tracking-widest">Navigate</h4>
            <ul className="space-y-2.5 list-none p-0">
              {[
                { href: '#hero', label: 'Home' },
                { href: '#about', label: 'About & Features' },
                { href: '#apis', label: 'APIs & Tech' },
                { href: '#developer', label: 'Developer' },
              ].map(({ href, label }) => (
                <li key={href}>
                  <a
                    href={href}
                    onClick={(e) => {
                      e.preventDefault();
                      handleNav(href);
                    }}
                    className="text-slate-500 hover:text-amber-400 transition-colors duration-300 text-sm no-underline"
                  >
                    {label}
                  </a>
                </li>
              ))}
              <li>
                <Link
                  to="/privacy-policy"
                  className="text-slate-500 hover:text-amber-400 transition-colors duration-300 text-sm no-underline flex items-center gap-1.5"
                >
                  <Lock size={13} className="text-amber-500/60" />
                  Privacy Policy
                </Link>
              </li>
            </ul>
          </div>

          {/* Links */}
          <div>
            <h4 className="text-white font-semibold mb-4 text-sm uppercase tracking-widest">Download & Source</h4>
            <ul className="space-y-3 list-none p-0">
              <li>
                <a
                  href="https://play.google.com/store/apps/details?id=com.sk_altamash188.noor"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-slate-500 hover:text-amber-400 transition-colors duration-300 text-sm no-underline"
                  id="footer-playstore-link"
                >
                  <PlayIcon />
                  Google Play Store
                </a>
              </li>
              <li>
                <a
                  href="https://github.com/SkAltmash"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 text-slate-500 hover:text-amber-400 transition-colors duration-300 text-sm no-underline"
                  id="footer-github-link"
                >
                  <GithubIcon size={16} />
                  GitHub: SkAltmash
                </a>
              </li>
              <li className="text-slate-600 text-xs font-mono mt-4">
                Package: com.sk_altamash188.noor
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom bar */}
        <div
          className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-white/5"
        >
          <p className="text-slate-600 text-xs">
            © {year} Noor App · Developed by{' '}
            <a
              href="https://github.com/SkAltmash"
              target="_blank"
              rel="noopener noreferrer"
              className="text-amber-500/70 hover:text-amber-400 transition-colors no-underline"
            >
              Sk Altamash
            </a>
            {' '}·{' '}
            <Link
              to="/privacy-policy"
              className="text-amber-500/70 hover:text-amber-400 transition-colors no-underline"
            >
              Privacy Policy
            </Link>
          </p>
          <div className="flex items-center gap-1 text-slate-600 text-xs">
            <span>Made with</span>
            <span className="text-rose-500 mx-0.5">♥</span>
            <span>for the Muslim Ummah</span>
          </div>
        </div>
      </div>
    </footer>
  );
}
